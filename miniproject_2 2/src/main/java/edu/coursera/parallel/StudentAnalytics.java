...
public final class StudentAnalytics {
	/**
	 * Sequentially computes the average age of all actively enrolled students using
	 * loops.
	 *
	...
	/**
	 * TODO compute the average age of all actively enrolled students using parallel
	 * streams. This should mirror the functionality of
	 * averageAgeOfEnrolledStudentsImperative. This method should not use any loops.
	 *
	 * @param studentArray Student data for the class.
	 * @return Average age of enrolled students
	 */
	public double averageAgeOfEnrolledStudentsParallelStream(final Student[] studentArray) {
		...
		return Stream.of(studentArray).filter(a -> a.checkIsCurrent()).mapToDouble(s -> s.getAge()).average()
				.getAsDouble();
	}

	/**
	 * Sequentially computes the most common first name out of all students that are
	 * no longer active in the class using loops.
	 *
	 * @param studentArray Student data for the class.
	 ...
	 * functionality of mostCommonFirstNameOfInactiveStudentsImperative. This method
	...
	public String mostCommonFirstNameOfInactiveStudentsParallelStream(final Student[] studentArray) {
		return Stream.of(studentArray).parallel().filter(a -> !a.checkIsCurrent()).map(s -> s.getFirstName())
				.collect(Collectors.groupingBy(Function.identity(), Collectors.counting())).entrySet().parallelStream()
				.max(Map.Entry.comparingByValue()).get().getKey();
	}

	/**
	 * Sequentially computes the number of students who have failed the course who
	 * are also older than 20 years old. A failing grade is anything below a 65. A
	 * student has only failed the course if they have a failing grade and they are
	...
	/**
	 * TODO compute the number of students who have failed the course who are also
	 * older than 20 years old. A failing grade is anything below a 65. A student
	 * has only failed the course if they have a failing grade and they are not
	 * currently active. This should mirror the functionality of
	 * countNumberOfFailedStudentsOlderThan20Imperative. This method should not use
	 * any loops.
	 *
	 * @param studentArray Student data for the class.
	 * @return Number of failed grades from students older than 20 years old.
	...
	public int countNumberOfFailedStudentsOlderThan20ParallelStream(final Student[] studentArray) {
		return Long.valueOf(Stream.of(studentArray).parallel()
				.filter(s -> !s.checkIsCurrent() && s.getAge() > 20 && s.getGrade() < 65).count()).intValue();
