...
public final class ReciprocalArraySum {
	...
	private static class ReciprocalArraySumTask extends RecursiveAction {
		final static int THREESHOLD = 1000000;
		...
		protected void compute() {
			int numberElements = endIndexExclusive - startIndexInclusive;
			int poolSize = pool.getPoolSize();
			int queueSize = pool.getParallelism();

			if (numberElements <= THREESHOLD || queueSize == poolSize) {
				for (int i = startIndexInclusive; i < endIndexExclusive; i++) {
					value += 1 / input[i];
				}
			} else {

				ReciprocalArraySumTask left = new ReciprocalArraySumTask(startIndexInclusive, numberElements / 2,
						input);
				ReciprocalArraySumTask right = new ReciprocalArraySumTask((numberElements / 2), endIndexExclusive,
						input);
	...
	protected static double parManyTaskArraySum(final double[] input, final int numTasks) {
		...
		pool = new ForkJoinPool(numTasks);
		// Compute sum of reciprocals of array elements
...
//		}
//		return sum;
		ReciprocalArraySumTask recursiveTask = new ReciprocalArraySumTask(0, input.length, input);
		pool.invoke(recursiveTask);
		return recursiveTask.getValue();
